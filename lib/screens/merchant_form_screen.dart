// lib/screens/merchant_form_screen.dart
...
class _MerchantFormScreenState extends State<MerchantFormScreen> {
  ...
  // Nouveaux contrôleurs pour les champs supplémentaires
    ...
    setState(() {
      if (pickedFile != null) {
        if (type == 'photoEnseigne') {
          _photoEnseigne = File(pickedFile.path);
        } else if (type == 'cniRecto') {
          _cniRecto = File(pickedFile.path);
        } else if (type == 'cniVerso') {
          _cniVerso = File(pickedFile.path);
        } else if (type == 'photoPasseport') {
          _photoPasseport = File(pickedFile.path);
        }
      }
    });
  }
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (_photoEnseigne != null) {
        request.files.add(await http.MultipartFile.fromPath('photoEnseigne', _photoEnseigne!.path));
      }
      if (_selectedIdType == 'cni' && _cniRecto != null && _cniVerso != null) {
        request.files.add(await http.MultipartFile.fromPath('pieceIdentiteRecto', _cniRecto!.path));
        request.files.add(await http.MultipartFile.fromPath('pieceIdentiteVerso', _cniVerso!.path));
      } else if (_selectedIdType == 'passeport' && _photoPasseport != null) {
        request.files.add(await http.MultipartFile.fromPath('photoPasseport', _photoPasseport!.path));
      }
  ...
  Widget _buildImagePreview(File? file, String label) {
    return Padding(
      ...
      child: Row(
        children: [
          Icon(file != null ? Icons.check_circle : Icons.cancel, color: file != null ? Colors.green : Colors.red),
          const SizedBox(width: 8),
          Expanded(
            child: Text(file != null ? 'Fichier sélectionné: ${file.path.split('/').last}' : 'Aucun fichier pour: $label'),
          ),
        ],
      ),
    );
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              // Section : Informations sur l'activité commerciale
              Container(
              ...
              // Section : Informations du gérant et fiscales
              Container(
                decoration: kCardDecoration,
                padding: const EdgeInsets.all(16.0),
                margin: const EdgeInsets.only(bottom: 20),
              ...
              // === Section : Informations de l'opérateur ===
              Container(
                decoration: kCardDecoration,
                padding: const EdgeInsets.all(16.0),
                margin: const EdgeInsets.only(bottom: 20),
